    ...
    BOOST_FOREACH(const CTxIn& txin, tx.vin)
    {
        ...
        else
        {
            ...
            if(!GetTransaction(txin.prevout.hash, prevTx, prevTx_hashBlock))
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "No infromaton avaliable about prev transaction");
            nValue = prevTx.vout[txin.prevout.n].nValue;
            CTxDestination addr;
            if(ExtractDestination(prevTx.vout[txin.prevout.n].scriptPubKey, addr))
                strAddress = CBitcoinAddress(addr).ToString();
        }
        in.push_back(Pair("sequence", (int64_t)txin.nSequence));
        in.push_back(Pair("value", ValueFromAmount(nValue)));
        in.push_back(Pair("address", strAddress));
        vin.push_back(in);
    }
