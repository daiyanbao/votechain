...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinUnits::parse(BitcoinUnits::ELC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
{
    // Convert elcoin:// to elcoin:
    //
    //    Cannot handle this later, because elcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("elcoin://"))
    {
        uri.replace(0, 12, "elcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "elcoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for elcoin.lnk
...
{
    return GetAutostartDir() / "elcoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write an elcoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=elcoin\n";
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Elcoin") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  elcoin [" + tr("command-line options") + "]                     " + "\n";
